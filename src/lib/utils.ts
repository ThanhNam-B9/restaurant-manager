...
export const setAccessTokenAndRefreshTokenToLocalStorage = (token: {
  refreshToken: string;
  accessToken: string;
}) => {
  isBrowser && localStorage.setItem("refreshToken", token.refreshToken);
  isBrowser && localStorage.setItem("accessToken", token.accessToken);
};
