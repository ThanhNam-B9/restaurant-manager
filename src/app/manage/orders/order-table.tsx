...
import { useGetOrderList, useUpdateOrder } from "@/queries/useOrder";
...
export default function OrderTable() {
  ...
  const refreshOrder = orderListQuery.refetch;
  ...
  useEffect(() => {
    ...
    function newOrders(data: GuestCreateOrdersResType["data"]) {
      const { guest } = data[0];
      toast({
        description: `Khách "${guest?.name}" tại bàn ${guest?.tableNumber} vừa đặt ${data.length} món `,
      });
      refetch();
    }
    function onPayment(data: PayGuestOrdersResType["data"]) {
      const { guest, quantity } = data[0];
      console.log("ws", data);

      toast({
        description: `Quý khách "${guest?.name}" tại bàn ${guest?.tableNumber} đã thanh toán thành công ${quantity} món`,
      });
      refetch();
    }
  ...
  }, [refreshOrder, fromDate, toDate, socket]);
  return (
