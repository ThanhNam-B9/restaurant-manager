...
import { useDeleteTable, useTableList } from "@/queries/useTable";
import QRCodeTable from "@/components/qrcode-tables";
...
export const columns: ColumnDef<TableItem>[] = [
  {
    ...
    filterFn: (row, columnId, filterValue) => {
      ...
      return filterValue === String(row.getValue("number"));
    },
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div
      // className="bg-blue-100 p-4 flex flex-col gap-4 w-[220px]"
      >
        <QRCodeTable
        ...
        />
        {/* <div>
          <span className="text-black">Bàn số {row.getValue("number")}</span>
          <h4 className="text-black">Quét QR để đặt bàn</h4>
        </div> */}
      </div>
    ),
  },
...
function AlertDialogDeleteTable({
  ...
  const daleteTableMutation = useDeleteTable();
  const handleDeleteTable = async () => {
    if (tableDelete) {
      if (daleteTableMutation.isPending) return;
      try {
        const res = await daleteTableMutation.mutateAsync(tableDelete.number);
        toast({
...
export default function TableTable() {
  ...
  const { data: listTablesQuery } = useTableList();
  // const params = Object.fromEntries(searchParam.entries())
  const [tableIdEdit, setTableIdEdit] = useState<number | undefined>();
  const [tableDelete, setTableDelete] = useState<TableItem | null>(null);
  const data = listTablesQuery?.payload.data ?? [];
